...
//					WriterUtil.mkdirs(newFile);
	...
	public double getScoreForSentence(String sentence1,String sentence2)
	{
		try{
			...
			String query_word1_word2 = "select count(tmp.article) from ( select distinct(r.article) as article from reversal r,word w where w.name in ('"+w1_str+"') and r.word = w.id ) tmp where tmp.article in ( select distinct(r.article) from reversal r,word w where w.name in ('"+w2_str+"') and r.word = w.id )";
			String query_count_article = "select count(*) from article";
