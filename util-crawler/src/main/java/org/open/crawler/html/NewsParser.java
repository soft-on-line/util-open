...
public class NewsParser extends org.open.util.debug.Test {
	...
	private Map<Node, TextConsum> containerMap                   = new HashMap<Node, TextConsum>();
	...
	public static final int       NEWS_BODY_MIN_COMMON_TEXT_SUM  = 20;                                                                                                                                     // 新闻正文最少字数
	public static final double    NEWS_BODY_MAX_LINKDEN_SITYRATE = 0.45;                                                                                                                                   // 新闻正文最多包含链接比率
	public static final int       NEWS_TITLE_MIN_SAME_CHAR       = 5;                                                                                                                                      // 新闻标题于title内容最少匹配字数
	public static final int       NEWS_BODY_MAX_SERES_LINK_NODE  = 5;                                                                                                                                      // 新闻标题于title内容最少匹配字数
	public static final int       NEWS_BODY_COPYRIGHT_POST       = 500;                                                                                                                                    // 版权信息离时间节点需大于此数值（节点数位置）

	public static String[]        NEWS_DATETIME_REGEXP_LIB       = { "([0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日[0-9]{1,2}:[0-9]{1,2})", "([0-9]{4}/[0-9]{1,2}/[0-9]{1,2}\\s[0-9]{1,2}:[0-9]{1,2})", "([0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日 [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2})",
	        "([0-9]{4}-[0-9]{1,2}-[0-9]{1,2} [0-9]{2}:[0-9]{2}:[0-9]{2})", "([0-9]{4}-[0-9]{1,2}-[0-9]{1,2} [0-9]{2}:[0-9]{2})", "([0-9]{4}年[0-9]{1,2}月[0-9]{1,2}日)", "([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})" };

	/**
	 * 文本计数对象
	 * @author peng
	 */
	protected class TextConsum {
	...
	private boolean _parseNewsBody() {
		...
		if (txtContainer1 != null) {
			if (containerMap.get(txtContainer1).getLinkDensityRate() > NEWS_BODY_MAX_LINKDEN_SITYRATE || containerMap.get(txtContainer1).getCommonTextSum() < NEWS_BODY_MIN_COMMON_TEXT_SUM) {
				return false;
			}
			...
			if (txtContainer2 != null) {
				if ((containerMap.get(txtContainer2).getCommonTextSum() > textConsum.getCommonTextSum() / 2) && (containerMap.get(txtContainer2).getLinkDensityRate() < NEWS_BODY_MAX_LINKDEN_SITYRATE)) // 次结点内容也比较多
				{
			...
			if (title.length() > 0) {
				if (newsBodyNode != null) {
					...
					for (int i = start + 2; i <= titleCheckPos && i < textNodeList.size(); i++) {
						int sameSum = 0;
						if (textNodeList.get(i).getNodeType() == Node.TEXT_NODE && textNodeList.get(i) != nodelist.item(0).getFirstChild()) {
							String nodeValue = textNodeList.get(i).getNodeValue().trim();
							sameSum = StringUtil.getSameCharSum(title, nodeValue);
							if (sameSum > max && sameSum > NEWS_TITLE_MIN_SAME_CHAR && nodeValue.length() <= title.length()) {
	...
	private boolean _isCopyright(Node node) {
		...
		if (RegexpUtil.isMatch(content, "\\s*[C|c]opyright")) {
			...
			if (pos >= NEWS_BODY_COPYRIGHT_POST) {
				return true;
			}
	...
	private void _removeNoise() {
		List<Node> bodynodelist = new ArrayList<Node>();
		Map<Node, TextConsum> bodycontainerMap = new HashMap<Node, TextConsum>();
	...
	private void _buildMap(Node node, Map<Node, TextConsum> map) {
		...
		while (parentNode != null) {
			if (parentNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (element.getTagName().equals("TABLE") || element.getTagName().equals("DIV") || element.getTagName().equals("TBODY") || element.getTagName().equals("BODY")) // 找到父节点容器
				{
